...
-- ============================================================================
-- Revoke anon EXECUTE on submit_video
-- ============================================================================
-- submit_video is SECURITY DEFINER (runs as the function owner, bypassing RLS),
-- so granting EXECUTE to anon lets anyone with the public anon key call the RPC
-- directly against the Supabase URL — bypassing the API route entirely and
-- defeating its rate limiting (checkRateLimit, 5 per minute).
--
-- Public submissions go through POST /api/public/submit, which uses the
-- service_role client (createServiceSupabase), and admin submissions use the
-- authenticated client — so anon never needs EXECUTE on this function.
--
-- Changes:
--   1. Revoke the anon grant on the categories-array submit_video overload.

REVOKE EXECUTE ON FUNCTION public.submit_video(text, text, text, text, text, text, text, timestamptz, text, text[]) FROM anon;
-- ============================================================================
-- Revoke anon EXECUTE on get_videos_for_api
-- ============================================================================
-- get_videos_for_api is the admin-only videos list RPC. It is SECURITY
-- INVOKER, so an anon caller would be constrained by RLS (anon can only see
-- published, non-trashed videos), meaning this is not a data leak. However,
-- the public frontend never calls it — it uses get_frontend_videos instead —
-- so the anon grant is unnecessary surface area and should be removed.
--
-- Changes:
--   1. Revoke the anon grant on get_videos_for_api.

REVOKE EXECUTE ON FUNCTION public.get_videos_for_api(jsonb) FROM anon;
-- ============================================================================
-- increment_video_view — raise when the video does not exist
-- ============================================================================
-- Data-integrity hardening. The function previously ran
--   UPDATE ... WHERE id = p_video_id
-- and silently succeeded even when no row matched, so a view increment for a
-- deleted/non-existent video was recorded as a success. Now an IF NOT FOUND
-- check raises an exception so callers (POST /api/public/views and
-- POST /api/auth/views) surface a failure instead of a false ok.
--
-- Security note: the RPC remains SECURITY DEFINER but is granted only to
-- authenticated and service_role (never anon), so direct abuse is not
-- possible — the API routes apply rate limiting / auth first.
--
-- CREATE OR REPLACE preserves the existing EXECUTE grants, so no re-grants
-- are needed for this same-signature rewrite.
...
$function$;
-- ============================================================================
-- Fix get_frontend_category_videos — apply LIMIT before jsonb_agg
-- ============================================================================
-- Performance bug: the LATERAL subquery aggregated ALL matching rows into one
-- JSON array with jsonb_agg, and only then applied `LIMIT p_per_category`.
-- Since jsonb_agg returns exactly one row, the LIMIT was a no-op — a category
-- with thousands of published videos would load and serialize every one of
-- them over the wire just to return p_per_category (4) for the homepage.
--
-- Fix: wrap the row selection in an inner subquery that orders and LIMITs
-- BEFORE the aggregate, so only p_per_category rows are ever materialized.
--
-- CREATE OR REPLACE preserves the existing EXECUTE grants (anon,
-- authenticated), so no re-grants are needed for this same-signature rewrite.
  ...
  FROM (
    ...
    LEFT JOIN LATERAL (
      ...
      FROM (
        ...
        WHERE v.status = 'published'
          AND slugs.slug = ANY(v.categories)
        ORDER BY v.video_post_date DESC NULLS LAST, v.created_at DESC
        LIMIT p_per_category
      ) v
    ) sub ON true
  ) combined;
$function$;
-- ============================================================================
-- Optimize get_frontend_videos — narrow projection before window sort
-- ============================================================================
-- Same fix as 20260801000002 (admin get_videos_for_api), applied to the
-- public frontend RPC. The old query did SELECT * in the filtered CTE and
-- COUNT(*) OVER() on every matching row, so Postgres held heavy text columns
-- (description, tags, categories, long URLs) in memory for ALL matching
-- videos just to sort + count them — even the thousands dropped by the
-- pagination offset.
--
-- Fix: scan/sort/count only id + the sort key + created_at in the window
-- pass, then JOIN back to the full video rows only for the returned page.
-- Also caps per_page (default 72) and floors page at 1, mirroring the admin
-- RPC guard rails.
--
-- CREATE OR REPLACE preserves the existing EXECUTE grants (anon,
-- authenticated), so no re-grants are needed for this same-signature rewrite.
...
BEGIN
  ...
  EXECUTE format(
    'WITH filtered AS (
      ...
      FROM public.videos v
      WHERE v.status = ''published''
        AND ($1 IS NULL OR $1 = ANY(v.categories))
        AND ($2 IS NULL OR v.location = $2)
        AND ($3 IS NULL OR v.tags @> ARRAY[$3])
        AND ($4 IS NULL
          OR v.description ILIKE ''%%'' || $4 || ''%%''
          OR v.city ILIKE ''%%'' || $4 || ''%%''
          OR v.location ILIKE ''%%'' || $4 || ''%%'')
    ),
    counted AS (
      SELECT id, sort_key, created_at, COUNT(*) OVER() AS total_count
      FROM filtered
    ...
    SELECT jsonb_build_object(
      ''videos'', COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          ...
          ''video_post_date'', p.video_post_date,
          ''view_count'', p.view_count,
          ''status'', p.status,
          ''created_at'', p.created_at
        ) ORDER BY c.sort_key %2$s, p.created_at DESC)
        FROM counted c
        JOIN public.videos p ON p.id = c.id
      ), ''[]''::jsonb),
      ''total'', COALESCE((SELECT total_count FROM counted LIMIT 1), 0)
...
$$;
-- ============================================================================
-- Optimize get_dashboard_stats — single-pass scan via MATERIALIZED CTE
-- ============================================================================
-- The previous function ran five independent scalar subqueries, each scanning
-- the full videos table (heap reads pull large text columns like description
-- and video_url into shared buffers even though they are not selected):
--   total_videos, trashed_count, status_counts, category_counts,
--   location_counts.
--
-- Fix: a MATERIALIZED CTE extracts only the four small columns needed
-- (is_trashed, status, categories, location) in exactly one table pass, then
-- all five aggregations run against that tiny in-memory dataset — one heap
-- scan instead of five, no cache thrashing from large text columns.
--
-- Output shape is unchanged (total_videos, trashed_count, status_counts,
-- category_counts, location_counts), so the dashboard page and its
-- unstable_cache / revalidateDashboardStats flow are unaffected.
--
-- CREATE OR REPLACE preserves the existing EXECUTE grants (authenticated,
-- service_role), so no re-grants are needed for this same-signature rewrite.
...
AS $function$
  ...
  SELECT jsonb_build_object(
    'total_videos', (SELECT COUNT(*)::int FROM base),

    'trashed_count', (SELECT COUNT(*)::int FROM base WHERE is_trashed),
    ...
    'category_counts', COALESCE((
      ...
      FROM (
        SELECT unnest(categories) AS slug, COUNT(*)::int AS cnt
        FROM base
        WHERE categories IS NOT NULL AND NOT is_trashed
        GROUP BY 1
      ) c
    ), '[]'::jsonb),
    ...
    'location_counts', COALESCE((
      ...
      FROM (
        SELECT location, COUNT(*)::int AS cnt
        FROM base
        WHERE location IS NOT NULL AND NOT is_trashed
        GROUP BY location
      ) v
    ), '[]'::jsonb)
  );
$function$;
-- ============================================================================
-- Optimize get_homepage_stats — single-pass scan via MATERIALIZED CTE
-- ============================================================================
-- Similar to get_dashboard_stats, get_homepage_stats ran 4 independent full-table
-- scans. This applies the same MATERIALIZED CTE pattern to scan the videos table
-- exactly once and calculate all aggregates efficiently.
...
AS $function$
  ...
  SELECT jsonb_build_object(
    'total_videos',  (SELECT COUNT(*)::int FROM base),
    'total_cities',  (SELECT COUNT(DISTINCT city)::int FROM base WHERE city IS NOT NULL),
    'total_locations', (SELECT COUNT(DISTINCT location)::int FROM base WHERE location IS NOT NULL),
    'location_counts', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('slug', v.location, 'count', v.cnt) ORDER BY v.cnt DESC)
      FROM (
        SELECT location, COUNT(*)::int AS cnt 
...
$function$;
-- ============================================================================
-- Drop orphaned submit_video function
-- ============================================================================
-- When the 10-parameter version of submit_video was introduced (adding
-- p_categories text[]), the old 9-parameter version was left behind.
-- Postgres treats them as two separate functions, and the old one was
-- writing to the dropped `category` column layout.

DROP FUNCTION IF EXISTS public.submit_video(text, text, text, text, text, text, text, timestamptz, text);
-- ============================================================================
-- Add IF NOT FOUND checks to trash_video and restore_video
-- ============================================================================
-- Ensure that trashing or restoring a non-existent video properly raises
...
-- ============================================================================
-- These indexes are fully covered by composite indexes (like idx_videos_admin_list,
-- idx_videos_status_post, idx_videos_status_city) or have extremely low 
-- cardinality (video_src), causing unnecessary write overhead.

DROP INDEX IF EXISTS public.idx_videos_status;
DROP INDEX IF EXISTS public.idx_videos_city;
DROP INDEX IF EXISTS public.idx_videos_location;
DROP INDEX IF EXISTS public.idx_videos_video_src;
DROP INDEX IF EXISTS public.idx_videos_ig_post_date;
-- ============================================================================
-- Escape ILIKE wildcards in search queries
-- ============================================================================
-- Prevents a user from passing '%' or '_' in the search query and triggering
-- a slow full-index regex-like scan. Both get_videos_for_api and 
-- get_frontend_videos are updated to sanitize the search parameter.
...
BEGIN
  ...
  safe_search := replace(replace(replace(search_query, '\', '\\'), '%', '\%'), '_', '\_');
  ...
  USING trashed_filter, status_filter, category_filter, location_filter, safe_search, per_page, offset_count, page_num, per_page;
...
BEGIN
  ...
  -- Escape LIKE wildcards
  safe_search := replace(replace(replace(p_query, '\', '\\'), '%', '\%'), '_', '\_');
  ...
  USING p_category, p_location, p_tag, safe_search, p_per_page, v_offset;
